...
CREATE TABLE test_user (
  ...
  def test_insert(self):
    ...
    self.assertEqual(
      1, self.db.insert("test_user", {"username": "def", "password": "secret"})
    )

  def test_bulk_insert_works_with_multiple_formats(self):
    columns_as_tuples = ("username", "password")
    rows_as_tuples_list = [("abc", "secret"), ("def", "secret")]

    columns_as_list = ["username", "password"]
    rows_as_lists_list = [["ghf", "secret"], ["ijk", "secret"]]

    rows_as_lists_tuple = (("lmn", "secret"), ("opq", "secret"))

    self.assertEqual(
      2, self.db.bulk_insert("test_user", columns_as_tuples, rows_as_tuples_list)
    )
    self.assertEqual(
      2, self.db.bulk_insert("test_user", columns_as_list, rows_as_lists_list)
    )
    self.assertEqual(
      2, self.db.bulk_insert("test_user", columns_as_list, rows_as_lists_tuple)
    )

  def test_update(self):
    self.db.insert("test_user", username="abc", password="secret")
    self.assertEqual(
      1, self.db.update("test_user", {"password": "supersecret"}, {"username": "abc"})
    )
    self.assertEqual(
      0, self.db.update("test_user", {"password": "None"}, {"username": None})
    )
  ...
  def test_get_row(self):
    self.db.insert(
      "test_user", username="abc", password="secret", created_at=datetime.datetime.now()
    )
    ...
    self.assertTrue(
      dict, self.db.get_row("SELECT * FROM test_user", output_type="dict")
    )
    self.assertTrue(
      dict, self.db.get_row("SELECT * FROM test_user", output_type="object")
    )
  ...
  def test_get_col(self):
    ...
    self.db.insert("test_user", {"username": "def", "password": "secret"})
    rows = self.db.get_col(
      "SELECT username, password FROM test_user", column_offset="password"
    )
  ...
  def test_injection(self):
    ...
    self.assertEqual(
      ...
      self.db.update(
        ...
        {"username": "abc'; DELETE FROM test_user;"},
      ),
    )
